...
const OrderHistoryData = () => {
    ...
    onSubmit: async (values) => {
      try {
        const response = await axios.get(
          `${server}/order/getAllOrders?paymentStatus=${values.paymentStatus}&fromDate=${values.from}&toDate=${values.to}`
        );
        setOrders(response.data.orders);
        setFilteredOrders(response.data.orders);
  ...
  <div className="flex flex-wrap -mx-3 items-end">
    <div className="w-full sm:w-1/4 px-3 mb-4 sm:mb-0">
      <label
        className="block uppercase tracking-wide text-gray-700 text-xs sm:text-xs font-bold mb-2"
        htmlFor="from"
      >
        From *
      </label>
      <input
        ...
        className="appearance-none block w-full bg-gray-200 text-gray-700 border rounded py-2 px-3 sm:py-3 sm:px-4 leading-tight focus:outline-none focus:bg-white"
      />
    ...
    <div className="w-full sm:w-1/4 px-3 mb-4 sm:mb-0 flex justify-end">
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        <div className="flex">
          {Array.from(
            { length: Math.ceil(filteredOrders.length / entriesPerPage) },
            (_, i) => (
              <button
                key={i}
