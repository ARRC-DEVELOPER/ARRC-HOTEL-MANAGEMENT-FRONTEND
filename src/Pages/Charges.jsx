...
const Charges = () => {
  ...
  const handleSave = async (values, { resetForm }) => {
    try {
      if (editingCharge) {
        await axios.put(
          `${server}/charges/updateCharge/${editingCharge._id}`,
          values
        );
        ...
        await axios.post(`${server}/charges/createCharge`, values);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${server}/charges/deleteCharge/${id}`);
    ...
    <div className="p-6">
      ...
      <table className="min-w-full bg-white border border-gray-300">
        <thead>
          <tr>
            {columns.map((col) => (
              <th key={col.key} className="p-2 border-b txet-center">
                {col.title}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {filteredData.map((charge, index) => (
            <tr key={charge._id}>
              {columns.map((col) => (
                <td key={col.key} className="p-2 border-b text-center">
                  {col.render
                    ? col.render(charge[col.dataIndex], charge, index)
                    : charge[col.dataIndex]}
                </td>
              ))}
            </tr>
          ))}
          ...
          <div className="bg-white p-6 rounded-md shadow-lg">
            ...
            <Formik
              initialValues={{
                ...
                value: editingCharge ? editingCharge.value : 0.0,
              }}
